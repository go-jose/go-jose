...
func TestSampleNimbusJWEMessagesRSA(t *testing.T) {
	...
	// RSA1_5 vectors (not available in FIPS mode)
	rsa15Messages := []string{
	...
	// RSA-OAEP (SHA-1) vectors (not available in FIPS mode)
	rsaOAEPMessages := []string{
	...
	// RSA-OAEP-256 vectors (FIPS compatible)
	rsaOAEP256Messages := []string{
	...
	rsaSampleMessages := make([]string, 0, 18)
	if !fips140.Enabled() {
		rsaSampleMessages = append(rsaSampleMessages, rsa15Messages...)
		rsaSampleMessages = append(rsaSampleMessages, rsaOAEPMessages...)
	}
	rsaSampleMessages = append(rsaSampleMessages, rsaOAEP256Messages...)

	allowedKeyAlgs := []KeyAlgorithm{RSA_OAEP_256}
	if !fips140.Enabled() {
		allowedKeyAlgs = append(allowedKeyAlgs, RSA1_5, RSA_OAEP)
	}
