...
// excludedEd25519PublicKeys contains low-order and non-canonical Ed25519
// encodings that must not be accepted as JOSE verification keys.
var excludedEd25519PublicKeys = [][ed25519.PublicKeySize]byte{
	{
...
func validateEd25519PublicKey(publicKey ed25519.PublicKey) error {
	...
	for _, excluded := range excludedEd25519PublicKeys {
		if bytes.Equal(publicKey, excluded[:]) {
